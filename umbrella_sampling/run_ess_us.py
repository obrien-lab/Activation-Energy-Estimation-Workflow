...
def parse_input(cntrl_file):
    ...
    if tag_error:
        ...
        return(top, reactant_cor, np, qm_mask, qm_charge, reaction_coordinate, 
               rc_list, temp, min_step, sampling_step, force_constant, if_wham, only_wham,
               wham_exec, wham_rc_interval, extra_restraint, position_restraint)

def check_log_file():
    if not os.path.exists('ess_us.log'):
        return 1
    elif not os.path.getsize('ess_us.log'):
        return 1
...
def ess_jobs(idx, rc_restraints, extra_restraints, pos_restraints):
    global top, reactant_cor, np, qm_mask, qm_charge, min_step
    
    f_log = open('../ess_us.log', 'a')
    f_log.write('ess_%d\n'%(idx+1))
    f_log.close()
    ...
    f = open('min.in', 'w')
    f.write('''LBFGS minimization in QM/MM with increasing restraints
 &cntrl
  imin   = 1,
  maxcyc = '''+str(int(min_step))+''',
  ncyc = '''+str(int(min_step/2))+''',
  cut=10.0, 
  ntb=1, 
  ntc=1, 
  ntf=1,''')
 ...
 &qmmm
  ...
  qm_ewald=1, qm_pme=1,
 /
&wt type='REST', istep1=0, istep2='''+str(int(min_step/2))+''', value1=0.1, value2=1.0,/
&wt type='REST', istep1='''+str(int(min_step/2)+1)+''', istep2='''+str(int(min_step))+''', value1=1.0, value2=1.0,/
&wt type='END' /
DISANG=RST.dist
''')
    ...
    f.close()
    if pos_restraints == []:
        os.system('mpirun -np '+str(int(np))+' sander.MPI -O -i min.in -p ../'+top+' -c '+crd+' -r ess_'+str(idx+1)+'.ncrst -o ess_'+str(idx+1)+'.out -inf mdinfo.log')
    else:
        os.system('mpirun -np '+str(int(np))+' sander.MPI -O -i min.in -p ../'+top+' -c '+crd+' -r ess_'+str(idx+1)+'.ncrst -o ess_'+str(idx+1)+'.out -ref '+crd+' -inf mdinfo.log')


def us_jobs(idx, rc_restraints, extra_restraints, pos_restraints):
    global top, np, temp, qm_mask, qm_charge, sampling_step
    
    f_log = open('../ess_us.log', 'a')
    f_log.write('us_%d\n'%(idx+1))
    f_log.close()
    
    crd = '../ess/ess_%d.ncrst'%(idx+1)
...
&wt type='END' /
DISANG=RST.dist
DUMPAVE=dist_'''+str(idx+1)+'''.dat
''')
    
    f.close()
    if pos_restraints == []:
        os.system('mpirun -np '+str(int(np))+' sander.MPI -O -i us.in -p ../'+top+' -c '+crd+' -x us_'+str(idx+1)+'.nc -r us_'+str(idx+1)+'.ncrst -o us_'+str(idx+1)+'.out -inf mdinfo.log')
    else:
        os.system('mpirun -np '+str(int(np))+' sander.MPI -O -i us.in -p ../'+top+' -c '+crd+' -x us_'+str(idx+1)+'.nc -r us_'+str(idx+1)+'.ncrst -o us_'+str(idx+1)+'.out -ref '+crd+' -inf mdinfo.log')

def get_PMF(rc_list, force_constant, skip_steps):
    global wham_exec, temp, wham_rc_interval
    tol = 1e-4
...
(top, reactant_cor, np, qm_mask, qm_charge, reaction_coordinate, 
 rc_list, temp, min_step, sampling_step, force_constant, if_wham, only_wham, wham_exec, 
...
for i in range(n_windows):
    if stage <= i+1:
        ...
        restraints = '%s r1=%.4f, r2=%.4f, r3=%.4f, r4=%.4f, rk2=%.4f, rk3=%.4f'%(
                     restraints_head, rc-999, rc, rc, rc+999, 1000, 1000)
        os.chdir('ess')
        ess_jobs(i, restraints, extra_restraints, position_restraint)
        os.chdir('../')
...
if (stage <= 2*n_windows+1 and if_wham == 1) or only_wham == 1:
    get_PMF(rc_list, force_constant, int(sampling_step/2))
    f = open('wham/pmf.dat')
    ff = open('pmf.dat', 'w')
    tag = 0
    pmf_list = []
